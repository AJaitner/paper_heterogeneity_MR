...
setting_num = 8
parameter_matrix = matrix(NA, nrow = setting_num, ncol = 13)

parameter_matrix[,1] = rep(0,setting_num) #par.U0
parameter_matrix[,2] = rep(c(0,0,0.8,0.8),2) # par.UG
parameter_matrix[,3] = rep(-2,setting_num) # par.S0
parameter_matrix[,4] = c(rep(0,4), rep(0.8,4)) # par.SG
parameter_matrix[,5] = rep(0.8,setting_num) # par.SG2 
parameter_matrix[,6] = rep(1.6,setting_num) # par.SU
parameter_matrix[,7] = rep(3500,setting_num) # par.Y0
parameter_matrix[,8] = rep(c(0,80,0,80),2) # par.YG
parameter_matrix[,9] = rep(80,setting_num) # par.YU
parameter_matrix[,10] = rep(0,setting_num) # par.YG2
parameter_matrix[,11] = rep(0,setting_num) # par.UG2
parameter_matrix[,12] = rep(-200,setting_num) # beta.1
parameter_matrix[,13] = rep(-100,setting_num) #beta.0
...
rgmte_est = matrix(NA, nrow =N, ncol = dim(parameter_matrix_dtf)[1])
beta_0_est = matrix(NA, nrow =N, ncol = dim(parameter_matrix_dtf)[1])
sd_rgmte = matrix(NA, nrow =N, ncol = dim(parameter_matrix_dtf)[1])
sd_beta_0 = matrix(NA, nrow =N, ncol = dim(parameter_matrix_dtf)[1])

fstat_stage_1_MR = matrix(NA, nrow =N, ncol = dim(parameter_matrix_dtf)[1])

pval_rgmte = matrix(NA, nrow =N, ncol = dim(parameter_matrix_dtf)[1])
