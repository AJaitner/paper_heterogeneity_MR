...
lm_Y_sen1_sum_beta_1 = matrix(NA, nrow =N, ncol = length(n))
lm_Y_sen1_sum_beta_0 = matrix(NA, nrow =N, ncol = length(n))
sd_lm_beta_1 = matrix(NA, nrow =N, ncol = length(n))
sd_lm_beta_0 = matrix(NA, nrow =N, ncol = length(n))

pval_beta_1 = matrix(NA, nrow =N, ncol =length(n))
pval_beta_0 = matrix(NA, nrow =N, ncol = length(n))

fstat_stage_1_MR_G = matrix(NA, nrow =N, ncol = length(n))
fstat_stage_1_MR_G2 = matrix(NA, nrow =N, ncol = length(n))
...
var_beta1_beta0_per_n = matrix(NA, nrow =N, ncol = length(n))
sd_beta1_beta0_per_n = matrix(NA, nrow =N, ncol = length(n))
...
for(k in 1:length(n)){
  for(i in 1:N){
    ...
    lm_Y_sen1_sum_beta_1[,k] = sapply(lm_Y_sen1_sum, function(x) x$coefficients[2,1])
    ...
    sd_lm_beta_1[,k] = sapply(lm_Y_sen1_sum, function(x) x$coefficients[2,2])
    ...
    pval_beta_1[,k] = sapply(lm_Y_sen1_sum, function(x) x$coefficients[2,4])
