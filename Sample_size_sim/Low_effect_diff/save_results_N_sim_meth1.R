...
# -------------------------------------------------------------------------------------------------------------------
beta1_beta0_matrix_scen1 = lm_Y_sen1_sum_beta_1 - lm_Y_sen1_sum_beta_0

res_table = cbind(n, scen$beta_1[s], scen$beta_0[s],scen$beta_1[s]-scen$beta_0[s],
mean(fstat_stage_1_MR_G2), mean(fstat_stage_1_MR_G),
mean(lm_Y_sen1_sum_beta_0, na.rm = TRUE),  sd(lm_Y_sen1_sum_beta_0, na.rm = TRUE), 
mean(lm_Y_sen1_sum_beta_1, na.rm = TRUE), sd(lm_Y_sen1_sum_beta_1, na.rm = TRUE), 
mean(beta1_beta0_matrix_scen1, na.rm = TRUE), sd(beta1_beta0_matrix_scen1,na.rm = TRUE))
...
test_stat = abs(beta1_beta0_matrix_scen1/sd_beta1_beta0_per_n) %>% as_tibble()
pval_diff = map_df(test_stat, function(x) pmin(1, 2*pmin(pnorm(x), 1-pnorm(x)))) 

res_table$power_rgmte = power_cal(pval_diff)
...
# beta 0 ---------------------------------------------------------------------------------------------------------
beta_0_lower_CI = lm_Y_sen1_sum_beta_0 - qnorm(0.975)*sd_lm_beta_0
beta_0_higher_CI = lm_Y_sen1_sum_beta_0 + qnorm(0.975)*sd_lm_beta_0
...
# beta 1 ---------------------------------------------------------------------------------------------------------
beta_1_lower_CI = lm_Y_sen1_sum_beta_1 - qnorm(0.975)*sd_lm_beta_1
beta_1_higher_CI = lm_Y_sen1_sum_beta_1 + qnorm(0.975)*sd_lm_beta_1
